...
public class CalendarPane extends MicrobaComponent implements
     ...
     * @throws PropertyVetoException if the date is restricted by the current
     * {@link VetoPolicy}.
     * @see #getVetoPolicy()
    ...
    /**
     * getLocalDate, Get a joda-time LocalDate from the date picker. A LocalDate
     * is a "human readable" date object that does not store any time zone, or
     * any time of day. It only stores a month, day, year, and era (CE/BCE).
     *
     * This function is designed to work without regards to time zones. In other
     * words, if the date picker displays Jan 1, 2001, then that is always the
     * date that will be returned in the LocalDate object.
     *
     * See also:
     * http://joda-time.sourceforge.net/apidocs/org/joda/time/LocalDate.html 
     */
    public LocalDate getLocalDate() {
        // Get the time zone from the date picker.
        DateTimeZone pickerZone = DateTimeZone.forTimeZone(this.getZone());
        // Convert the date picker date to a LocalDate, without any
        // changes to the date due to time zones.
        Date javaDate = this.getDate();
        Instant jodaInstant = new Instant(javaDate);
        DateTime jodaDateTime = new DateTime(jodaInstant, pickerZone);
        LocalDate jodaLocalDate = jodaDateTime.toLocalDate();
        // Return the LocalDate.
        return jodaLocalDate;
    }

    /**
     * setLocalDate, Set the date picker from a joda-time LocalDate. A LocalDate
     * is a "human readable" date object that does not store any time zone, or
     * any time of day. It only stores a month, day, year, and era (CE/BCE).
     *
     * This function is designed to work without regards to time zones. In other
     * words, if the LocalDate contains Jan 1, 2001, then that is always the
     * date that will be displayed in the date picker.
     *
     * Implementation Details: The exact time in the date picker will be set to
     * the first moment of the specified day. This moment is typically midnight,
     * but it could be 1am depending on any daylight savings time jumps in the
     * DatePicker's internal time zone. This will not affect the accurate
     * display or a subsequent getLocalDate() retrieval of the supplied date.
     *
     * See also:
     * http://joda-time.sourceforge.net/apidocs/org/joda/time/LocalDate.html and
     * LocalDate.toDateTimeAtStartOfDay(DateTimeZone zone)
     */
    public void setLocalDate(LocalDate aLocalDate) {
        try {
            setLocalDateWithThrow(aLocalDate);
        } catch (PropertyVetoException e) {
        }
    }

    /**
     * setLocalDateWithThrow, Set the date picker from a joda-time LocalDate,
     * with the possibility that a PropertyVetoException might be thrown based
     * on a currently set date veto policy.
     *
     * See the setLocalDate() function documentation for additional details.
     */
    public void setLocalDateWithThrow(LocalDate aLocalDate)
            throws PropertyVetoException {
        // Get the time zone from the date picker.
        DateTimeZone pickerZone = DateTimeZone.forTimeZone(this.getZone());
        // Convert the local date to a Java Date object, without any
        // changes to the date due to time zones.
        DateTime jodaDateTime = aLocalDate.toDateTimeAtStartOfDay(pickerZone);
        Instant jodaInstant = jodaDateTime.toInstant();
        Date javaDate = jodaInstant.toDate();
        // Set the date picker date.
        this.setDate(javaDate);
    }
     ...
     * The today-button allows the user to quickly select current date.
     *
     * @param visible <code>true</code> to show the today-button
     ...
     * date).
     *
     * @param visible <code>true</code> to show the none-button
     * <code>false</code> to hide
     */
     ...
     * Original meaning preserved.
     *
     * @param behavior the focus lost behavior constant
     * @see JFormattedTextField
     */
     ...
     *
     * @param holidayPolicy a holliday policy to set. May be <code>null</code>
     * @see VetoPolicy
     */
     ...
     * The policy is used to veto dates in the control.
     *
     * @param vetoModel a veto policy to set. May be <code>null</code>
     */
     ...
     * locale and ime zone?
     *
     * @return <code>true</code> if {@link #getDate()} returns a stripped date,
     * <code>false</code> otherwise
     ...
     * keep it.
     *
     * @param stripTime <code>true</code> to strip time, <code>false</code> to
     * keep time
     */
     ...
     *
     * @param commit <code>true</code> to indicate commit, <code>false</code> to
     * indicate revert
     * @see CommitEvent
     * @see CommitListener
     */
     ...
     * @param zone time zone to get zero fields in
     * @param locale locale to base the calendar on
